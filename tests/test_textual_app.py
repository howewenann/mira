...
class TextualAppTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_goal_and_rubric_result_render_as_dedicated_blocks(self) -> None:
        app = make_app()
        goal = goal_artifact(
            goal_id="goal-1", title="Ranked search", objective="Add ranked search.",
            success_criteria="- Results are ranked.\n- Focused tests pass.",
            rubric_enabled=True, rubric_iterations=3,
        )
        ...
        async with app.run_test(size=(100, 35)) as pilot:
            ...
            goal_blocks = list(chat.query(".goal"))
            self.assertEqual(len(goal_blocks), 1)
            goal_text = renderable_plain(goal_blocks[0].query_one(".goal-body", Static))
            self.assertIn("GOAL", goal_text)
    ...
    async def test_goal_command_uses_transient_staging_without_switching_mode(self) -> None:
        ...
        async def run_goal(**kwargs: Any) -> None:
            captured.update(dict(kwargs["mode"]["goal_staging"]))
        ...
        self.assertIsNone(app.mode["goal_staging"])
        self.assertEqual(captured["authoritative_objective"], "Write a professional announcement.")
        self.assertEqual(captured["stage"], "goal_research")
    ...
    async def test_goal_close_retains_lifecycle_and_current_goal(self) -> None:
        app = make_app()
        value = goal_artifact(goal_id="goal-1", title="Work", objective="Do work.", success_criteria="- Work is done.", rubric_enabled=False, rubric_iterations=3)
        async with app.run_test(size=(100, 30)) as pilot:
            await pilot.pause()
            app.session["current_goal"] = value
            app.mode["current_goal"] = value
    ...
    async def test_goal_command_declining_incomplete_plan_replacement_preserves_plan(self) -> None:
        ...
        plan = plan_artifact(
            plan_id="plan-1",
            title="Existing Plan",
            objective="Keep this work.",
            context_and_constraints="None.",
            key_changes=["Keep it."],
            test_plan=["Verify it."],
            assumptions=["No additional assumptions."],
            success_criteria="- It remains current.",
            rubric_enabled=False,
            rubric_iterations=3,
        )
        app._prompt_choice = AsyncMock(return_value="keep")
    ...
    async def test_plan_prompt_suffix_enters_mode_and_uses_normal_turn_path(self) -> None:
        app = make_app()
        captured: dict[str, Any] = {}
        ...
        with patch("core.application.session.run_user_turn", fake_run_user_turn):
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                await app.submit_prompt(
                    PromptBox.Submitted(prompt, '/plan   "keep quoted text"')
                )
                await wait_until(lambda: not app.busy)

        self.assertTrue(app.mode["planning"])
        self.assertEqual(app.mode["plan_thread_id"], "thread-1:plan")
        self.assertEqual(captured["text"], '"keep quoted text"')
    ...
    async def test_settings_panel_renders_default_rows_and_closes(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                self.assertEqual(response_status_input.styles.width, rubric_input.styles.width)
    ...
    async def test_rubric_setting_change_keeps_goal_snapshot(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                goal = goal_artifact(goal_id="goal-1", title="Search", objective="Add search.", success_criteria="- Search works.", rubric_enabled=False, rubric_iterations=3)
                app.session["current_goal"] = goal
                app.mode["current_goal"] = goal
                await pilot.pause()
                ...
                self.assertTrue(ok)
                self.assertEqual(app.session["current_goal"]["id"], "goal-1")
                self.assertFalse(app.session["current_goal"]["rubric_enabled"])
    ...
    async def test_tool_section_headers_match_system_spacing(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(100, 55)) as pilot:
                ...
                self.assertEqual(len(custom_header.query(".settings-column-label.plan")), 0)
                panel.query_one("#settings-tab-mcp", Button).press()
                await pilot.pause()
    ...
    async def test_settings_access_recycles_one_body_and_counts_only_effective_tools(self) -> None:
        """Access should omit disabled tools, count explicit grants, and persist toggles."""
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            workspace = Path(directory)
            configured = set_tool_plan_access(load_settings(workspace), "custom_search", True)
            configured = set_tool_rubric_access(configured, "custom_search", False)
            configured["hitl"]["tools"]["disabled_custom"] = {
                "enabled": False,
                "ptc": True,
                "rubric": True,
            }
            configured = set_mcp_tool_policy_value(configured, "docs", "search", "rubric", True)
            configured.setdefault("mcp", {}).setdefault("servers", {}).setdefault("off", {})["enabled"] = False
            configured = set_mcp_tool_policy_value(configured, "off", "hidden", "rubric", True)
            ...
            off = SimpleNamespace(
                ...
                usable=True,
                tools=[object()],
                tool_metadata=[{"original_name": "hidden"}],
            )
            ...
            async with app.run_test(size=(110, 35)) as pilot:
                ...
                app.mcp_manager = SimpleNamespace(
                    servers={"docs": docs, "off": off},
                    show_status=False,
                    shutdown=AsyncMock(),
                )
                app.mode.setdefault("resources", {})["tools"] = [
                    {"name": "custom_search", "source": "project"},
                    {"name": "disabled_custom", "source": "project"},
                ]
                app._handle_settings_command("access")
                await wait_until(lambda: len(app.query(SettingsPanel)) == 1)
                panel = app.query_one(SettingsPanel)
                await wait_until(lambda: len(panel.query("#settings-access-policy-rubric")) == 1)
                await pilot.pause()
                self.assertEqual(
                    renderable_plain(panel.query_one(".settings-access-landing-heading", Static)),
                    "Tool Access",
                )
                access_heading = panel.query_one(".settings-access-landing-heading", Static)
                self.assertEqual(access_heading.styles.color, Color.parse("#D8DEE9"))
                self.assertEqual(access_heading.styles.margin.top, 2)
                self.assertIsNone(app.screen.focused)
                self.assertEqual(
                    renderable_plain(panel.query_one(".settings-access-description", Static)),
                ...
                self.assertEqual(plan_button.styles.width, execute_select.styles.width)
                self.assertEqual(plan_button.styles.height, execute_select.styles.height)
                ptc_button = panel.query_one("#settings-access-policy-ptc", Button)
                self.assertEqual(ptc_button.region.height, 3)
                self.assertEqual(ptc_button.styles.margin.bottom, 0)
                self.assertEqual(ptc_button.styles.border_top, ("solid", Color.parse("#56616a")))
                ptc_button.focus()
                await pilot.pause()
                self.assertEqual(ptc_button.styles.border_top, ("solid", Color.parse("#5bb8b1")))
                self.assertEqual(ptc_button.styles.background, Color.parse("#151a1d"))
                self.assertNotIn("reverse", str(ptc_button.styles.text_style))

                panel.query_one("#settings-access-policy-rubric", Button).press()
                await wait_until(lambda: len(panel.query("#settings-toggle-rubric_access-custom_search")) == 1)
                await pilot.pause()
                switcher = panel.query_one("#settings-access-switcher", ContentSwitcher)
                self.assertEqual(switcher.current, "settings-access-detail")
                detail = panel.query_one("#settings-access-detail")
                back = panel.query_one("#settings-access-back", Button)
                self.assertTrue(back.compact)
                self.assertEqual(str(back.label), "< Back")
                self.assertEqual(back.region.height, 1)
                self.assertTrue(all(edge[0] == "" for edge in back.styles.border))
                back.focus()
                await pilot.pause()
                self.assertEqual(back.styles.background, Color.parse("#1b3036"))
                self.assertEqual(back.styles.color, Color.parse("#eef7f8"))
                self.assertNotIn("reverse", str(back.styles.text_style))
                self.assertEqual(
                    renderable_plain(detail.query_one(".settings-access-heading", Static)),
                    "Rubric Access",
                )
                detail_heading = detail.query_one(".settings-access-detail-heading", Static)
                self.assertEqual(detail_heading.styles.color, Color.parse("#D8DEE9"))
                self.assertEqual(detail_heading.styles.margin.top, 1)
                self.assertEqual(back.styles.margin.top, 1)
                guidance = panel.query_one(".settings-access-guidance", Static)
                self.assertEqual(
                    renderable_plain(guidance),
                    "Only enabled tools are shown. Choose which tools the Rubric verifier can use; "
                ...
                for header in headers:
                    ...
                    self.assertEqual(header.styles.margin.bottom, 0)
                ...
                for read_tool in READ_ONLY_BUILTIN_TOOLS:
                    editable = panel.query_one(f"#settings-toggle-rubric_access-{read_tool}", Button)
                    self.assertEqual(str(editable.label), "yes")
                    self.assertFalse(editable.disabled)
                for blocked_tool in ("write_file", "edit_file", "delete", "eval", "task"):
                    fixed = panel.query_one(
                        f"#settings-access-fixed-rubric-{blocked_tool}", Static
                    )
                    self.assertEqual(renderable_plain(fixed), "blocked")
                    self.assertEqual(fixed.styles.color, Color.parse("#7A858C"))
                    self.assertEqual(fixed.styles.background.a, 0)
                    self.assertEqual(len(panel.query(f"#settings-toggle-rubric_access-{blocked_tool}")), 0)
                ...
                self.assertEqual(
                    str(panel.query_one("#settings-toggle-mcp_tool_rubric-docs-search", Button).label),
                    "yes",
                )
                panel.query_one("#settings-toggle-rubric_access-custom_search", Button).press()
                await wait_until(lambda: tool_rubric_access(load_settings(workspace), "custom_search"))
                await pilot.pause()
                self.assertEqual(
                    str(panel.query_one("#settings-access-policy-rubric", Button).label),
                    panel._access_policy_label("rubric"),
                )
                self.assertTrue(
                ...
                await wait_until(lambda: switcher.current == "settings-access-landing")
                await pilot.pause()
                self.assertIsNone(app.screen.focused)

                panel.query_one("#settings-access-policy-rubric", Button).press()
                await wait_until(lambda: switcher.current == "settings-access-detail")
                await wait_until(
                    lambda: panel.query_one("#settings-access-back", Button).has_focus
                )
                await pilot.press("escape")
                await wait_until(lambda: switcher.current == "settings-access-landing")
    ...
    async def test_settings_access_plan_shows_editable_reads_and_static_blocked_actions(self) -> None:
        """Plan Access should keep one aligned list of editable reads and blocked actions."""
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            workspace = Path(directory)
            configured = set_tool_enabled(load_settings(workspace), "execute", True)
            configured = set_tool_plan_access(configured, "read_file", False)
            self.assertTrue(save_settings(workspace, configured))
            app = make_app(workspace=workspace, config={"settings": configured})
            ...
            async with app.run_test(size=(100, 35)) as pilot:
                ...
                panel.query_one("#settings-access-policy-plan", Button).press()
                await wait_until(lambda: len(panel.query("#settings-access-back")) == 1)
                await pilot.pause()
                detail = panel.query_one("#settings-access-detail")
                ...
                self.assertEqual(
                    renderable_plain(detail.query_one(".settings-access-guidance", Static)),
                    "Only enabled tools are shown. Read-only access can be changed; action tools "
                    "are blocked in Plan mode.",
                )
                ...
                for read_tool in READ_ONLY_BUILTIN_TOOLS:
                    editable = detail.query_one(f"#settings-toggle-plan_access-{read_tool}", Button)
                    self.assertEqual(str(editable.label), "no" if read_tool == "read_file" else "yes")
                    self.assertFalse(editable.disabled)
                first_editable = detail.query_one("#settings-toggle-plan_access-ls", Button)
                for blocked_tool in ("write_file", "edit_file", "delete", "execute", "eval", "task"):
                    fixed = detail.query_one(f"#settings-access-fixed-plan-{blocked_tool}", Static)
                    self.assertEqual(renderable_plain(fixed), "blocked")
                    self.assertEqual(fixed.region.x, first_editable.region.x)
                    self.assertEqual(len(detail.query(f"#settings-toggle-plan_access-{blocked_tool}")), 0)

                detail.query_one("#settings-toggle-plan_access-read_file", Button).press()
                await wait_until(lambda: tool_plan_access(load_settings(workspace), "read_file"))
    ...
    async def test_settings_access_rubric_keeps_reads_and_execute_editable(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            workspace = Path(directory)
            configured = set_tool_rubric_access(load_settings(workspace), "read_file", False)
            configured = set_tool_enabled(configured, "execute", True)
            configured = set_tool_rubric_access(configured, "execute", True)
            self.assertTrue(save_settings(workspace, configured))
            app = make_app(workspace=workspace, config={"settings": configured})
            ...
            async with app.run_test(size=(100, 35)) as pilot:
                ...
                read_file = panel.query_one("#settings-toggle-rubric_access-read_file", Button)
                execute = panel.query_one("#settings-toggle-rubric_access-execute", Button)
                self.assertEqual(str(read_file.label), "no")
                self.assertFalse(read_file.disabled)
                self.assertEqual(str(execute.label), "yes")
                self.assertFalse(execute.disabled)
                for blocked_tool in ("write_file", "edit_file", "delete", "eval", "task"):
                    self.assertEqual(
                        renderable_plain(
                            panel.query_one(f"#settings-access-fixed-rubric-{blocked_tool}", Static)
                        ),
                        "blocked",
                    )

                read_file.press()
                await wait_until(lambda: tool_rubric_access(load_settings(workspace), "read_file"))
    ...
    async def test_settings_panel_dims_saved_mcp_policies_when_tool_is_unavailable(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(120, 30)) as pilot:
                ...
                await wait_until(lambda: len(app.query(SettingsPanel)) > 0)
                panel = app.query_one(SettingsPanel)
                await wait_until(lambda: len(panel.query("#settings-tab-access")) == 1)
                panel.query_one("#settings-tab-access", Button).press()
                await pilot.pause()
    ...
    async def test_settings_panel_can_disable_inbuilt_tools(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                panel.query_one("#settings-access-policy-ptc", Button).press()
                await pilot.pause()
                self.assertEqual(len(panel.query("#settings-toggle-ptc-edit_file")), 0)
                self.assertEqual(len(calls), 1)
    ...
    async def test_settings_panel_ptc_is_independent_and_eval_gates_controls(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            configured = set_tool_ptc(configured, "grep", False)
            self.assertTrue(save_settings(workspace, configured))
            app = make_app(workspace=workspace, config={"settings": configured})
            ...
            async with app.run_test(size=(110, 35)) as pilot:
                ...
                for read_tool in READ_ONLY_BUILTIN_TOOLS:
                    editable = panel.query_one(f"#settings-toggle-ptc-{read_tool}", Button)
                    self.assertEqual(str(editable.label), "no" if read_tool == "grep" else "yes")
                    self.assertFalse(editable.disabled)
                for gateway in ("eval", "task"):
                    inapplicable = panel.query_one(f"#settings-access-fixed-ptc-{gateway}", Static)
                    self.assertEqual(renderable_plain(inapplicable), "-")
                    self.assertEqual(len(panel.query(f"#settings-toggle-ptc-{gateway}")), 0)
                self.assertEqual(
                    renderable_plain(panel.query_one(".settings-access-guidance", Static)),
                    "Only enabled tools are shown. Choose which tools can be called "
                    "programmatically through eval.",
                )
    ...
    async def test_settings_panel_can_disable_custom_tools(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                self.assertTrue(tool_plan_access(loaded, "project_status"))
                self.assertTrue(tool_ptc(loaded, "project_status"))
                panel.query_one("#settings-tab-access", Button).press()
                await pilot.pause()
                panel.query_one("#settings-access-policy-plan", Button).press()
                await pilot.pause()
                self.assertEqual(len(panel.query("#settings-toggle-plan_access-project_status")), 0)
                self.assertEqual(len(calls), 1)
    ...
    async def test_choice_prompt_buttons_wrap_in_narrow_viewports(self) -> None:
        ...
        async with app.run_test(size=(80, 40)) as pilot:
            ...
            await wait_until(lambda: all(row.region.height == 1 for row in panel.query(".prompt-panel-button-row")))
            await wait_until(
                lambda: list(panel.query(Button))[0].styles.background
                == PROMPT_BUTTON_FOCUS_BACKGROUND
            )
            rows = list(panel.query(".prompt-panel-button-row"))
            buttons = list(panel.query(Button))
