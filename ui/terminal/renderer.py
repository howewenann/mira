...
from ui.shared.terminal.transcript import DEFAULT_TOOL_OUTPUT_CHARS, TerminalTranscript
...
class Renderer:
    ...
    async def review_artifact(
        ...
        """Render a Core-owned formal artifact with the existing one-shot UX."""
        if kind == "plan":
            await self.finalize_plan(interrupt)
        else:
            self.transcript.block("goal", goal_artifact_text(artifact))
        return {"action": "close"}
